...
pub mod chain_spec;
#[path = "../../node/cli/src/service.rs"]
mod service;

use libp2p::wasm_ext;
use futures::{prelude::*, sync::oneshot, sync::mpsc};
use substrate_service::{ServiceFactory, RpcSession, Roles as ServiceRoles, Configuration};
use log::{debug, info};
...
fn start_inner(wasm_ext: wasm_ext::ffi::Transport) -> Result<Client, Box<dyn std::error::Error>> {
	...
	let chain_spec = chain_spec::flaming_fir_config().map_err(|e| format!("{:?}", e))?;
